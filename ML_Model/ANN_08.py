...
rng.shuffle(all_data, axis=1)
all_data[:, ::2] *= rng.normal(1, noise_std, all_data[:, ::2].shape)
...
model.save(f'saved_models/ANN_08_{tgt_time}')

print(result.history)

with open(f'saved_models/ANN_08_{tgt_time}/history.txt', "w") as file:
    file.write(str(result.history))

with open(f'saved_models/ANN_08_{tgt_time}/log.txt', "w") as file:
